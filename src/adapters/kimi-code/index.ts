...
/**
 * Kimi Code → AHS read-only projection (spike for ADR-0001 / ADR-0002).
 *
 ...
 *
 * MULTI-WIRE = MULTI-SESSION (the ADR-0002 validation case): each
 * agents/<name>/wire.jsonl is projected as its own AHS session. The main
 * agent's sessionId is the bare session uuid; a sub-agent's sessionId is
 * `<uuid>/<agent-name>`. state.json gives only an AGENT-level parent link
 * (parentAgentId — always "main" in observed data), so children get
 * relation spawned_by(parent session) with NO toolCallId anchor
 * (AC-0002-B-3).
 *
 ...
 * identical text (verified 1814/1814 exact matches on real data), and
 * append_message carries the better provenance marker (origin.kind).
 * turn.prompt is still counted for Manifest.stats.turnCount.
 *
 * Provenance (spec provenance principle — source HAS explicit markers here):
 * - append_message / turn input with origin.kind "user" → user_message.
 ...
 *   never silently lost). usageScope "session" (5 occurrences in real data,
 *   emitted at full-compaction time) is cumulative accounting, not a delta
 ...
 *   command, not the CreateGoal tool — 45 events vs 1 CreateGoal call in
 *   real data). goal.update WITH a status → goal_update verdict:
 *   "complete" → "met", "blocked" → "unmet", "paused" → "pending" (still
 *   open); goalId omitted (verdict events carry none — multiple verdicts
 *   correlate via seq, per spec). goal.update WITHOUT status
 *   ({turnsUsed}/{tokensUsed} progress telemetry, ~90% of goal events) and
 *   goal.clear (ambiguous removal, no verdict semantics) are DROPPED.
 ...
 * - forked events ({type:"forked", time} — 3 in real data) mark a session
 *   fork but carry NO source session id, so a forked_from relation cannot
 *   be populated: DROPPED (source-unavailable; flagged for ADR-0002).
 ...
 *   plan_mode.*, swarm_mode.*, turn.cancel, context.undo (1 occurrence;
 *   undone messages stay in the projection — known fidelity gap).
  ...
  private async discover(): Promise<SessionSource[]> {
    ...
    for (const wd of wdDirs) {
      ...
      for (const entry of entries) {
        ...
        sessions.push({ sessionId, dir, state, agents });
      }
    }
