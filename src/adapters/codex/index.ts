...
/**
 * Codex → AHS read-only projection (spike for ADR-0001 / ADR-0002).
 *
 ...
 *
 * Other lossy decisions (ADR-0001):
 ...
 *   dropped per ADR-0001 (same category as Claude's server_tool_use).
 * - token_count `rate_limits`, total-vs-last duplication: `last_token_usage`
 *   is the most recent API request's usage (its input = the full resent
 *   context, which is why it grows within a turn); `total_token_usage` is
 *   the authoritative cumulative cost. Codex re-emits token_count events
 *   with an UNCHANGED total (e.g. at turn boundaries) — these are
 *   duplicates and are skipped (verified on real data: with this dedup,
 *   sum(last_token_usage) == final total_token_usage for every session).
 * - event_msg process/telemetry subtypes (exec_command_end, patch_apply_end,
 *   update, sub_agent_activity, thread_settings_applied,
 *   thread_rolled_back, entered/exited_review_mode, item_completed, error):
 *   DROPPED.
 *
 * Sub-agents (CONTRADICTS the research doc, which says "no sub-agent
 * support"): current Codex spawns sub-agent threads. The child's rollout
 * file opens with its OWN session_meta whose `source` is
 * { subagent: { thread_spawn: { parent_thread_id, depth, agent_path } } }
 * (older ancestor session_meta lines follow as lineage headers — the first
 * session_meta always wins, and the parent content is NOT replayed).
 * This maps to Manifest.relation = spawned_by(parent_thread_id). The
 * toolCallId anchor is omitted (optional per spec): it is recoverable from
 * the parent's sub_agent_activity event (event_id = call id, agent_thread_id
 * = child thread id) via cross-file correlation — left as follow-up.
 *
 * Specific mappings:
 ...
 *   turn_aborted (observed in real data; the research doc does not list it)
 *   is mapped to turn_boundary end — the turn ended, by interruption.
...
/** rollout-<iso-ts>-<id>.jsonl → id (best effort; session_meta wins). */
function sessionIdFromFilename(fileName: string): string {
  const stem = fileName.replace(/^rollout-/, "").replace(/\.jsonl$/, "");
  // The ISO timestamp prefix uses '-' as date/time separator; the session id
  // (UUID with dashes, or ULID without) follows the time portion.
  const match = /^.{8}T.{6}-(.+)$/.exec(stem);
  return match?.[1] ?? stem;
}

function mapTokenUsage(raw: NonNullable<NonNullable<RawPayload["info"]>["last_token_usage"]>): Usage {
  const usage: Usage = {};
 ...
 */
export function projectRecords(sessionId: string, lines: RawLine[]): AhsRecord[] {
  ...
  for (const line of lines) {
    ...
    if (line.type === "response_item") {
      ...
      // tool_search_call(+_output) (server-side tools) are dropped per
      // ADR-0001.
      ...
      // event_msg user_message / agent_message are duplicates of
      // response_item messages — skipped. All other subtypes
      // (exec_command_end, patch_apply_end, sub_agent_activity, ...) are
      // process/telemetry — dropped per ADR-0001.
...
function buildManifest(
  ...
  let meta: RawPayload | undefined;
  let model: string | undefined;
  let workspaceRoots: string[] | undefined;
  let turnCount = 0;
  for (const line of lines) {
    const p = line.payload ?? {};
    if (line.type === "session_meta" && meta === undefined) meta = p;
  ...
  // Sub-agent spawn declaration → spawned_by relation. The toolCallId
  // anchor is omitted (recoverable from the parent's sub_agent_activity
  // event; left as follow-up).
  ...
  return {
    ...
    ...(parentThreadId !== undefined
      ? { relation: { type: "spawned_by" as const, sessionId: parentThreadId } }
      : {}),
    stats: {
      turnCount,
      ...(hasUsage ? { totalUsage } : {}),
...
export class CodexAdapter implements HarnessAdapter {
  ...
  private async load(filePath: string): Promise<SessionSource> {
    ...
    // ancestor lineage headers (resumed/forked threads) — first wins.
    const meta = lines.find((l) => l.type === "session_meta")?.payload;
    const sessionId =
      meta?.id ?? meta?.session_id ?? sessionIdFromFilename(path.basename(filePath));
    return { sessionId, filePath, lines };
  }

  private async *sessions(): AsyncIterable<SessionSource> {
    for (const filePath of await this.walk(this.basePath)) {
      yield await this.load(filePath);
    }
  }
  ...
  async *listSessions(filter?: SessionFilter): AsyncIterable<Manifest> {
    ...
    for await (const session of this.sessions()) {
      const records = projectRecords(session.sessionId, session.lines);
      // Files with zero projectable content are not AHS sessions — skip.
      if (records.length === 0) continue;
      const manifest = buildManifest(session.sessionId, session.lines, records);
  ...
  async *readRecords(sessionId: string): AsyncIterable<AhsRecord> {
    for await (const session of this.sessions()) {
      if (session.sessionId === sessionId) {
        yield* projectRecords(session.sessionId, session.lines);
        return;
      }
    }
