...
| AC-0002-N-4（usage 无静默丢失） | PASS — record 级 usage 求和与源总量**精确相等**（真实数据 sweep：input 541269 / output 54033 / cacheRead 1080272 / cacheWrite 0 / reasoning 0，两侧一致；源端无 currency → cost 整体 omit，不编造） | test + sweep |
...
| AC-0004-N-1（可用） | PASS — exportSessions → ahs-report 渲染 transcript（model_change 状态行、tool 调用、fork 折叠、--all 备选版本）；token 聚合精确 | test/pi-report.test.ts + test/e2e/adapter-chains.ts（pi 链） |
...
- 本机数据全部来自本地 ollama 模型，cost 在源端无 currency 字段 → cost 对象整体 omit
- `parentSession` 字段在 181 个文件中仅出现 1 次（已映射为 lineage，见下"契约摩擦"）
...
- **usage**：input/output/cacheRead/cacheWrite/reasoning 直译；源端无 currency 字段 → cost 对象整体 omit（不编造 currency）；`totalTokens` 冗余（分项之和）丢弃。
...
- **`session.parentSession`（跨文件 continue-from 指针）→ lineage**：~~调研文档未记录此字段（真实数据 181 文件中 1 例），此前按独立自足 session 投影、parentSession 丢弃。~~ **已解决**：`parentSession` 现映射为 `lineage: { type: "forked_from", sessionId: ... }`，跨文件指针保留为 lineage 边。真实合成 fixture 中可能无 parentSession 字段（合成数据无跨文件 continue-from），故 lineage 仅在源端存在 parentSession 时出现。
- **cost currency 源端缺失 → omit cost**：~~schema 要求 currency，源只给数值；此前按 USD 假设映射。~~ **已解决**：源端无 currency 字段时，cost 对象整体 omit（不编造 currency）。适配器不再假设 USD，golden fixture 与测试中的 cost 断言已同步移除。
