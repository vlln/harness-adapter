 ...
 * one stable-serialized { manifest, records } per listed session, generated
 * from the adapter and reviewed by hand. The fixtures are synthetic
 * (hand-crafted), so no sanitization concern (AC-0003-E-1).
 */
...
describe("claude-code golden (AC-0003-N-1)", () => {
  ...
  it("adapter output matches the reviewed golden files exactly", async () => {
    ...
    for (const session of sessions) {
      const goldenPath = path.join(goldenDir, `${session.manifest.sessionId}.json`);
